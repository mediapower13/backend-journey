...
const posts = createMemoryStore([
  { id: 1, title: 'Hello REST', content: 'First post', authorId: 1, published: true },
  { id: 2, title: 'Advanced Routing', content: 'Router modules', authorId: 2, published: false }
]);

router.get('/', (req, res) => {
  res.json({ version: 'v2', data: posts.list(), meta: { total: posts.list().length } });
});

router.get('/:id', (req, res, next) => {
  const post = posts.get(req.params.id);
  if (!post) return next(new NotFoundError('Post not found'));
  res.json({ version: 'v2', data: post });
});
...
router.post('/', (req, res, next) => {
  ...
  const created = posts.create({ title, content, authorId: authorId || null, published });
